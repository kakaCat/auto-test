...
│ │ 路径 [http://localhost:8002][/api/v1/____][解析GET参数]│ │
│ │ 系统 [用户系统 ▾]          模块 [用户管理 ▾]        │ │
│ │ 描述 [............. 多行文本 ..............]      │ │
│ │ 状态 [启用 ●○ 禁用]  需要登录 [需要认证 ●○ 公开访问] │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤
│ ▼ 请求参数 (3个参数)                                    │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ [📋 表格模式] [📝 JSON模式]  [📄 JSON导入] [📋 导出] │ │
│ │ ┌─ 参数表格 ─────────────────────────────────────┐   │ │
│ │ │ [+ 添加参数] [清空所有]                        │   │ │
│ │ │ 参数名 | 类型 | 必填 | 描述 | 操作               │   │ │
│ │ │ user_id | number | ✓ | 用户ID | [编辑][删除]    │   │ │
│ │ │ └─ profile | object | ○ | 用户资料 | [编辑][删除] │   │ │
│ │ │   └─ name | string | ✓ | 姓名 | [编辑][删除]    │   │ │
│ │ └───────────────────────────────────────────────┘   │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤
│ ▼ 响应配置 (5个字段)                                    │
...
│ │ │ [+ 添加字段] [清空所有]                        │   │ │
│ │ │ 字段名 | 类型 | 必返 | 描述 | 操作               │   │ │
│ │ │ code | number | ✓ | 状态码 | [编辑][删除]       │   │ │
│ │ │ data | object | ✓ | 数据 | [编辑][删除]         │   │ │
│ │ │ └─ id | number | ✓ | ID | [编辑][删除]         │   │ │
│ │ └───────────────────────────────────────────────┘   │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤
...
组件特性：
• 多步骤表单：分为4个标签页，支持步骤间切换
• 进度指示：实时显示表单完成度百分比
• 双模式编辑：参数和响应支持表格/JSON两种编辑模式
• 智能联动：系统选择后自动加载对应模块列表
• 实时校验：字段输入时进行即时验证和错误提示
• 数据导入导出：支持JSON格式的参数批量导入导出
```
