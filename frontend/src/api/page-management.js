/**
 * 页面管理API接口
 * Page Management API
 */

import { request } from '@/utils/request'

// 页面管理API
export const pageApi = {
  /**
   * 获取页面列表
   * @param {Object} params - 查询参数
   * @returns {Promise} 页面列表
   */
  getPages(params = {}) {
    return request.get('/pages/v1/', { params })
  },

  /**
   * 根据ID获取页面详情
   * @param {number} pageId - 页面ID
   * @returns {Promise} 页面详情
   */
  getPageById(pageId) {
    return request.get(`/pages/v1/${pageId}`)
  },

  /**
   * 创建页面
   * @param {Object} data - 页面数据
   * @returns {Promise} 创建结果
   */
  createPage(data) {
    return request.post('/pages/v1/', data)
  },

  /**
   * 更新页面
   * @param {number} pageId - 页面ID
   * @param {Object} data - 更新数据
   * @returns {Promise} 更新结果
   */
  updatePage(pageId, data) {
    return request.put(`/pages/v1/${pageId}`, data)
  },

  /**
   * 删除页面
   * @param {number} pageId - 页面ID
   * @returns {Promise} 删除结果
   */
  deletePage(pageId) {
    return request.delete(`/pages/v1/${pageId}`)
  },

  /**
   * 搜索页面
   * @param {Object} query - 搜索条件
   * @returns {Promise} 搜索结果
   */
  searchPages(query) {
    return request.post('/pages/v1/search', query)
  },

  /**
   * 简单搜索页面（GET方式）
   * @param {Object} params - 搜索参数
   * @returns {Promise} 搜索结果
   */
  searchPagesSimple(params = {}) {
    return request.get('/pages/v1/search/simple', { params })
  },

  /**
   * 获取页面的API列表
   * @param {number} pageId - 页面ID
   * @returns {Promise} API列表
   */
  getPageApis(pageId) {
    return request.get(`/pages/v1/${pageId}/apis`)
  },

  /**
   * 为页面添加API关联
   * @param {number} pageId - 页面ID
   * @param {Object} data - API关联数据
   * @returns {Promise} 添加结果
   */
  addPageApi(pageId, data) {
    return request.post(`/pages/v1/${pageId}/apis`, data)
  },

  /**
   * 更新页面API关联
   * @param {number} relationId - 关联ID
   * @param {Object} data - 更新数据
   * @returns {Promise} 更新结果
   */
  updatePageApi(relationId, data) {
    return request.put(`/page-apis/v1/${relationId}`, data)
  },

  /**
   * 删除页面API关联
   * @param {number} relationId - 关联ID
   * @returns {Promise} 删除结果
   */
  deletePageApi(relationId) {
    return request.delete(`/page-apis/v1/${relationId}`)
  },

  /**
   * 批量管理页面API关联
   * @param {number} pageId - 页面ID
   * @param {Object} data - 批量操作数据
   * @returns {Promise} 操作结果
   */
  batchManagePageApis(pageId, data) {
    return request.post(`/pages/v1/${pageId}/apis/batch`, data)
  },

  /**
   * 获取页面统计概览
   * @returns {Promise} 统计数据
   */
  getPagesStats() {
    return request.get('/pages/v1/stats/overview')
  },

  /**
   * 获取页面类型列表
   * @returns {Promise} 页面类型列表
   */
  getPageTypes() {
    return request.get('/pages/v1/types/list')
  },

  /**
   * 获取执行类型列表
   * @returns {Promise} 执行类型列表
   */
  getExecutionTypes() {
    return request.get('/pages/v1/execution-types/list')
  },

  /**
   * 批量更新页面状态
   * @param {Array} pageIds - 页面ID列表
   * @param {string} status - 目标状态
   * @returns {Promise} 更新结果
   */
  batchUpdatePageStatus(pageIds, status) {
    return request.post('/pages/v1/batch/status', { page_ids: pageIds, status })
  },

  /**
   * 批量启用页面
   * @param {Array} pageIds - 页面ID列表
   * @returns {Promise} 启用结果
   */
  batchEnable(pageIds) {
    return request.post('/pages/v1/batch/enable', { page_ids: pageIds })
  },

  /**
   * 批量禁用页面
   * @param {Array} pageIds - 页面ID列表
   * @returns {Promise} 禁用结果
   */
  batchDisable(pageIds) {
    return request.post('/pages/v1/batch/disable', { page_ids: pageIds })
  },

  /**
   * 批量删除页面
   * @param {Array} pageIds - 页面ID列表
   * @returns {Promise} 删除结果
   */
  batchDeletePages(pageIds) {
    return request.post('/pages/v1/batch/delete', { page_ids: pageIds })
  },

  /**
   * 批量导出页面
   * @param {Array} pageIds - 页面ID列表
   * @param {string} format - 导出格式
   * @returns {Promise} 导出结果
   */
  batchExport(pageIds, format = 'json') {
    return request.post('/pages/v1/batch/export', { page_ids: pageIds, format }, { responseType: 'blob' })
  },

  /**
   * 导入页面数据
   * @param {FormData} formData - 包含文件的表单数据
   * @returns {Promise} 导入结果
   */
  importPages(formData) {
    return request.post('/pages/v1/import/data', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  }
}

// 页面管理相关的工具函数
export const pageUtils = {
  /**
   * 格式化页面类型显示
   * @param {string} type - 页面类型
   * @returns {string} 显示文本
   */
  formatPageType(type) {
    const typeMap = {
      page: '页面',
      modal: '弹框',
      drawer: '抽屉',
      tab: '标签页',
      step: '步骤页'
    }
    return typeMap[type] || type
  },

  /**
   * 格式化页面状态显示
   * @param {string} status - 页面状态
   * @returns {string} 显示文本
   */
  formatPageStatus(status) {
    const statusMap = {
      active: '活跃',
      inactive: '非活跃',
      draft: '草稿'
    }
    return statusMap[status] || status
  },

  /**
   * 格式化执行类型显示
   * @param {string} type - 执行类型
   * @returns {string} 显示文本
   */
  formatExecutionType(type) {
    const typeMap = {
      parallel: '并行',
      serial: '串行'
    }
    return typeMap[type] || type
  },

  /**
   * 获取页面类型颜色
   * @param {string} type - 页面类型
   * @returns {string} 颜色类型
   */
  getPageTypeColor(type) {
    const colorMap = {
      page: '',
      modal: 'warning',
      drawer: 'info',
      tab: 'success',
      step: 'primary'
    }
    return colorMap[type] || ''
  },

  /**
   * 获取页面状态颜色
   * @param {string} status - 页面状态
   * @returns {string} 颜色类型
   */
  getPageStatusColor(status) {
    const colorMap = {
      active: 'success',
      inactive: 'info',
      draft: 'warning'
    }
    return colorMap[status] || ''
  },

  /**
   * 获取HTTP方法颜色
   * @param {string} method - HTTP方法
   * @returns {string} 颜色类型
   */
  getMethodColor(method) {
    const colorMap = {
      GET: 'success',
      POST: 'primary',
      PUT: 'warning',
      DELETE: 'danger',
      PATCH: 'info'
    }
    return colorMap[method] || ''
  },

  /**
   * 验证路由路径格式
   * @param {string} path - 路由路径
   * @returns {boolean} 是否有效
   */
  validateRoutePath(path) {
    if (!path) return true // 允许为空
    return /^\/[a-zA-Z0-9\-_\/]*$/.test(path)
  },

  /**
   * 验证JSON格式
   * @param {string} jsonStr - JSON字符串
   * @returns {boolean} 是否有效
   */
  validateJSON(jsonStr) {
    if (!jsonStr.trim()) return true
    try {
      JSON.parse(jsonStr)
      return true
    } catch {
      return false
    }
  },

  /**
   * 生成页面树形数据
   * @param {Array} pages - 页面列表
   * @param {Array} systems - 系统列表
   * @returns {Array} 树形数据
   */
  buildPageTree(pages, systems) {
    const tree = []
    
    systems.forEach(system => {
      const systemPages = pages.filter(page => page.system_id === system.id)
      
      const systemNode = {
        id: `system-${system.id}`,
        label: system.name,
        type: 'system',
        system_id: system.id,
        children: []
      }
      
      systemPages.forEach(page => {
        systemNode.children.push({
          id: `page-${page.id}`,
          label: page.name,
          type: 'page',
          page_id: page.id,
          system_id: system.id,
          page_type: page.page_type,
          status: page.status,
          api_count: page.api_count || 0
        })
      })
      
      tree.push(systemNode)
    })
    
    return tree
  }
}

export default {
  pageApi,
  pageUtils
}